...
def get_neet_countdown():
    ...
    now = datetime.now()
    if now > target: return "Exam Day! All the Best! 🎯"
    diff = target - now
    days = diff.days
...
def get_folder_ui(user_id, first_name, folder_id="root", is_admin=False):
    ...
    path_names.reverse()
    if folder_id == "root": path_text = "📂 <b>Pᴀᴛʜ:</b> Hᴏᴍᴇ"
    else:
        path_text = "📂 <b>Hᴏᴍᴇ</b>\n"
        space = ""
...
def reset_ui(message):
    ...
    bot.reply_to(message, "🛠️ <b>Emergency Fix Applied!</b>\nSabhi folders ka UI default par set ho gaya hai.", parse_mode='HTML')
...
def broadcast_msg(message):
    ...
    if text == "/broadcast": return bot.reply_to(message, "⚠️ Format: /broadcast <message>")
    ...
    bot.edit_message_text(f"✅ <b>Broadcast Complete!</b>\n\n🟢 Sent: {success}\n🔴 Failed: {failed}", chat_id=message.chat.id, message_id=msg.message_id, parse_mode='HTML')
...
def backup_db(message):
    ...
    with open("bot_backup.json", "w") as f: json.dump(data, f)
...
def handle_bulk_documents(message):
    ...
    if user_id in UPLOAD_STATE and user_id == ADMIN_ID:
        ...
        if message.document.mime_type == 'application/pdf':
            ...
            with open(input_pdf, 'wb') as new_file: new_file.write(downloaded_file)
            ...
            try:
                ...
                bot.send_message(message.chat.id, f"⚠️ Ye PDF corrupt ya locked hai. Bina watermark ke upload kar raha hoon...", parse_mode='HTML')
        ...
        else: bot.reply_to(message, "❌ Please send a valid PDF file only.")
...
def process_edit_bottom_text(message, folder_id, photo_id, title):
    ...
    test_caption = f"{title}\n\n<blockquote>👤 <b>Student:</b> Admin\n🆔 <b>User ID:</b> <code>12345</code>\n📂 <b>Current Folder:</b> Test Folder</blockquote>\n\n{bottom_text}"
    try:
        ...
        folders_col.update_one({"_id": folder_id}, {"$set": {"photo": photo_id, "title": title, "bottom_text": bottom_text}})
        ...
        bot.send_message(message.chat.id, f"❌ <b>HTML FORMATTING ERROR!</b> ❌\n\n<b>Error Details:</b> <code>{e}</code>", parse_mode='HTML')
...
def handle_poll_answer(poll_answer):
    ...
    scores_col.update_one({"_id": user_id}, {"$inc": {"score": points}, "$set": {"name": poll_answer.user.first_name}}, upsert=True)
                ...
                for user in all_users:
                    try:
                        poll_msg = bot.send_poll(
                            chat_id=user['_id'], question=f"⏱ 30 MIN CHALLENGE:\n\n{q['question']}", options=q['options'],
                            is_anonymous=False, type='quiz', correct_option_id=q['correct_option_id'], explanation="NEET Pattern: +4 for Correct, -1 for Wrong."
                        )
                        active_polls.insert_one({"poll_id": poll_msg.poll.id, "correct_option_id": q['correct_option_id'], "chat_id": user['_id'], "msg_id": poll_msg.message_id})
                    except: continue
            now = datetime.now()
            if now.hour == 0 and now.minute <= 30: send_leaderboard()
        except Exception as e: print(f"Quiz Loop Error: {e}")
        time.sleep(1800) 
...
def show_help_page(call):
    ...
    try: bot.edit_message_media(media=InputMediaPhoto(IMAGES['home'], caption=help_text, parse_mode='HTML'), chat_id=call.message.chat.id, message_id=call.message.message_id, reply_markup=markup)
    except: pass
    bot.answer_callback_query(call.id)
...
# ==========================================
# 🎓 STUDENT UI & DOWNLOAD (UPDATED CAPTIONS)
# ==========================================
...
def student_navigate(call):
    ...
    try: bot.edit_message_media(media=InputMediaPhoto(img, caption=caption, parse_mode='HTML'), chat_id=call.message.chat.id, message_id=call.message.message_id, reply_markup=markup)
    except: pass
    bot.answer_callback_query(call.id)

# 📌 SINGLE DOWNLOAD HANDLER (Updated Caption & Warning)
...
def get_paper(call):
    ...
    if paper:
        ...
        # SIRF EK WARNING BOX BANEGA AUR FILE KE SATH JAYEGA
        single_caption = (
            f"📄 <b>{paper['name']}</b>\n"
            f"━━━━━━━━━━━━━━━━━━━━\n"
            f"👤 <b>Uploaded by:</b> <a href='https://t.me/errorkidk'>ERROR</a>\n\n"
            f"<blockquote>🚨 <b>WARNING / ALERT:</b>\n"
            f"<i>Ye file <b>10 minute</b> me automatically delete ho jayegi. Kripya ise turant save karein!</i> ⏳</blockquote>"
        )
        
        sent_msg = bot.send_document(call.message.chat.id, paper['file_id'], caption=single_caption, parse_mode='HTML')
...
# 📌 BULK DOWNLOAD HANDLER (Updated to solve spam issue)
...
def bulk_download(call):
    ...
    for paper in papers:
        try:
            # 1. HAR PDF PAR SIRF USKA NORMAL NAME HOGA (Koi lamba text nahi)
            normal_caption = f"📄 <b>{paper['name']}</b>"
            sent_msg = bot.send_document(call.message.chat.id, paper['file_id'], caption=normal_caption, parse_mode='HTML')
            threading.Thread(target=delete_message_later, args=(call.message.chat.id, sent_msg.message_id)).start()
            time.sleep(0.3)
        except Exception as e:
            print(f"Error in Bulk Download: {e}")
            
    # 2. SAB FILES BHEJNE KE BAAD LAST ME EK WARNING MESSAGE AAYEGA
    final_warning = (
        f"✅ <b>Success! {len(papers)} files bhej di gayi hain.</b>\n"
        f"━━━━━━━━━━━━━━━━━━━━\n"
        f"👤 <b>Uploaded by:</b> <a href='https://t.me/errorkidk'>ERROR</a>\n\n"
        f"<blockquote>🚨 <b>WARNING / ALERT:</b>\n"
        f"<i>Security reasons ki wajah se ye sabhi files <b>10 minute</b> me automatically DELETED ho jayengi. Kripya inhe turant save ya kisi aur jagah forward kar lein!</i> ⏳</blockquote>"
    )
    bot.send_message(call.message.chat.id, final_warning, parse_mode='HTML')
